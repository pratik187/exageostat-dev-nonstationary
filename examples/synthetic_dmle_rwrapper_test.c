...
/**
 *
 * @file synthetic_dmle_rwrapper_test.c
 *
 * ExaGeoStat main function (R-Wrapper test).
 *
 * @version 1.2.0
 *
...
int main(int argc, char** argv) {
    ...
    arguments arguments;
    double* vecs_out = NULL, *miss_vecs = NULL, *theta_out = NULL, *zmiss = NULL, *globalzpredict = NULL, *fisher_out = NULL;
    ...
    fisher_out = (double* ) malloc(10 * sizeof(double));
    vecs_out = (double* ) malloc(globalveclen * sizeof(double));
    miss_vecs = (double* ) malloc(globalveclen * sizeof(double));
    ...
    gen_z_exact(&k, initial_theta, &num_params,
    ...
    //calcualte the fisher matrix
    fprintf(stderr, "Calculating the Fisher matrix......\n");
    fisher_general(vecs_out, &vecs_out[n], &n,
                   theta_out, &num_params,
                   &dts, &dm_int, &p_grid, &q_grid,
                   fisher_out);

    for (int iter = 0; iter < 9; iter++) {
        if (iter % 3 == 0)
            printf("\n");
        fprintf(stderr, " %6.4e ", fisher_out[iter]);
    }
    printf("\n");
    //data generation for the prediction function
    nZobs = n;
    seed = 5;   //any random value larger than 0
    zmiss = (double* ) malloc(nZmiss * sizeof(double));
    globalzpredict = (double* ) malloc(nZmiss * sizeof(double));

    if (nZmiss != 0) {
        gen_z_exact(&k, initial_theta,
                    &num_params,
                    &dm_int, &nZmiss, &seed, &ncores,
                    &gpus, &dts, &p_grid,
                    &q_grid, &globalveclen, miss_vecs);

        //prediction function
        int comp_mode = 0;

        exact_predict(vecs_out, NULL, &vecs_out[nZobs],
                      NULL, &vecs_out[2 * nZobs], NULL,
                      miss_vecs, NULL, &miss_vecs[nZmiss],
                      NULL, &nZobs, &nZmiss,
                      &k,
                      theta_out, &num_params,
                      &comp_mode, &dm_int, &ncores,
                      &gpus, &dts, &p_grid,
                      &q_grid, globalzpredict);

        double mspe = 0;
        for (i = 0; i < nZmiss; i++) {
            printf("(%f, %f): %f (actual: %f)\n", miss_vecs[i], miss_vecs[nZmiss + i], globalzpredict[i],
                   miss_vecs[2 * nZmiss + i]);
            mspe += pow(miss_vecs[2 * nZmiss + i] - globalzpredict[i], 2);
        }
        mspe = mspe / nZmiss;
        printf("Mean Sqaure Presiction Error (MSPE): %f\n", mspe);
    }
    rexageostat_finalize();

    printf("Estimated theta: \n");
