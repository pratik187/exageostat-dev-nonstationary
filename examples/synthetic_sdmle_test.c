...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < zvecs; i++) {
        ...
        if (nZmiss != 0) {
            ...
            if (data.mspe == 1) {
                ...
                for (j = 0; j < pred_samples; j++) {
                    ...
                    if (strcmp(data.computation, "exact") == 0 || strcmp(data.computation, "lr_approx") == 0)
                        prediction_error = EXAGEOSTAT_sdmle_Predict_Tile(&data, starting_theta, p * nZmiss, p * nZobs,
            ...
            if (strcmp(arguments.kernel_fun, "trivariate_matern_parsimonious_profile") == 0) {
                starting_theta[0] = data.variance1;
                starting_theta[1] = data.variance2;
