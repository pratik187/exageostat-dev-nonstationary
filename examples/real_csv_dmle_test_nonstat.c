...
int main(int argc, char **argv) {
    ...
    else if(strcmp(arguments.dim, "st") == 0)
    {
        ...
        //{
        //printf("%f, %f, %f\n",locations->x[index], locations->y[index], time_slots[index]);
        //}
        //return;
    }
    else
    ...
    if(p == 1)
    {
		...
		// data_split_nonstat_userDefined(l.x,l.y,l.x, &data, N, splitX,splitY,p);
        data_merge_nonstat_clustering(l.x,l.y,l.z, &data, N);
    ...
    else if(p == 3)
    {
        ...
        //locations_obs_zsort_inplace_trivariate(N/p, locations, streamdata1, streamdata2, streamdata3);
        //locations_obs_zsort_inplace(N/3, locations, streamdata1);
        //locations_obs_zsort_inplace(N/3, locations, streamdata2);
        //locations_obs_zsort_inplace(N/3, locations, streamdata3);
        streamdata=(double *) malloc(N * sizeof(double));
        int j=0;
        int i = 0;
        for(i=0;i<N/p;i++)
    ...
    if(data.mspe ==1)
    {
        ...
        for (j = 0; j < pred_samples; j++)
        {
            ...
            print_predicted_values( Zmiss, nZmiss);
    ...
    write_to_estimatedtheta( str, starting_theta, num_params, N/p, pred_time, all_time,
            (avg_pred_value1/=pred_samples), (avg_pred_value2/=pred_samples), (avg_pred_value3/=pred_samples), (avg_pred_value/=pred_samples), 
