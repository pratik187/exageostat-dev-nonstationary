...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < zvecs; i++) {
        ...
        if (strcmp(arguments.kernel_fun, "univariate_matern_non_gaussian") == 0 ||
            ...
            CHAMELEON_Tile_to_Lapack(data.descZ, l.z, N);
            free(lb);
            free(up);
            free(starting_theta);
            free(target_theta);
            ...
            if (data.mspe == 1) {
                ...
                for (j = 0; j < pred_samples; j++) {
                        ...
                        else if (strcmp (data.computation, "lr_approx") == 0)
                    {
                        ...
                        prediction_error = EXAGEOSTAT_TLR_dmle_ng_Predict_Tile(&data, starting_theta, p*nZmiss, p*nZobs, Zobs, Zactual, Zmiss, N, lts);
                    }
#endif
