...
CHAMELEON_Desc_Create_User(_desc_, _memspace_, _type2_, _mb_, _nb_, _mbXnb_, _lda_, _n_,_smb_, _snb_ , _m_, _n2_, \
        _p_, _q_, chameleon_getaddr_null, chameleon_getblkldd_ccrb, chameleon_getrankof_2d );    \
...
CHAMELEON_Desc_Create_User(_desc_, _memspace_, _type2_, _mb_, _nb_, _mbXnb_, _lda_, _n_,_smb_, _snb_ , _m_, _n2_, \
        _p_, _q_, chameleon_getaddr_null, chameleon_getblkldd_ccrb, chameleon_getrankof_2d_diag );   \
...
typedef struct {
    double *x;				///< X co-ordinates.
    double *y;				///< Y co-ordinates.
    double *z;                          ///< Values of Z.
    int index;                              
} full_data;
...
typedef struct {
    double *sigma_arr1;				///< Values in sigma array.
    double *lamda_arr1;				///< Values in lamda array.     
    double *sigma_arr2;				///< Values in sigma array.
    double *lamda_arr2;				///< Values in lamda array.       
    double *nu_arr1;
    double *nu_arr2;   
} non_stat_params;
...
typedef struct {
    ...
    double mse_pred3;
    double mse_pred;
    double trace_pred_sum;
    double trace_pred_mean;
...
typedef struct {
    ...
    double variance3;                 ///< Variance3 parameter.
    ...
    int* Nsplit;			          ///< Number of splits in nonstationary case.
    ...
    non_stat_params ns_params;        ///< ns_params for location.
    ...
    void *descZ3;                     ///< Measurements Z3 submatrix descriptor.
    ...
    void *descmse3;                   ///< Mean Square Error (MSE) descriptor.
    void *descZtrace;                 ///< Trace descriptor.
    void *sequence;                   ///< CHAMELEON sequence.
    void *request;                    ///< CHAMELEON request.
    ...
    double avg_flops_per_iter;        ///< Average flops per iteration (only used in verbose mode).
    ...
    char *obsFPath3;                  ///< Observations file path3 (bivariate case) --  in the case of real dataset (real mode).
    ...
    char *actualZFPath3;              ///< Actual observations file path -- in the case of prediction.
    char *actualZLocFPath;            ///< Actual locations file path -- in the case of prediction.
    char *actualTimeFPath;            ///< Actual time file path -- in the case of prediction (space-time kernel).
    double det;                       ///< Determinant value.
    double sum;                       ///< Sum value (non-Gaussian).
    double dotp;                      ///< Double dot product value.
    double dotp1;                     ///< Double dot1 product value.
    double dotp2;                     ///< Double dot2 product value.
    double dotp3;                     ///< Double dot3 product value.
    ...
    double mserror3;                  ///< Mean Square Error (MSE) value, variable 3 in case of bivariate.
 ...
 **/
#define VERBOSE(str)    \
    if (data->verbose == 1 && CHAMELEON_My_Mpi_Rank() == 0){    \
...
void write_prediction_result(char *path, int matrix_size, int no_missing,
                             double MSE1, double MSE2, double MSE3, double MSE,
...
int print_predicted_values(location *lmiss, double* Zactual,
                           double* Zmiss, int nZmiss, int p);

void shuffle3(double *array, double *array2, double *array3,
        location * locations, size_t n);
...
// double predict(double *x, double *y, double* Z_values, double startx, double endx, double starty, double endy, int N);
