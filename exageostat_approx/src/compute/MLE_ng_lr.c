...
int store_only_diagonal_ng_tiles = 1;
int print_index_ng = 0;
extern STARSH_blrf *mpiF;
int use_scratch_ng = 1;
int global_check_ng = 0;  //used to create dense matrix for accuracy check
int print_mat_ng = 0;
int calc_rank_stat_ng = 0; //Calculate the rank. Set to 0 in normal execution. 
double* Ark_initial_ng;
double* Ark_old_ng;
int flag2 = 0;

int EXAGEOSTAT_TLR_MLE_dzvg_ng_Tile(MLE_data *data, double* Nrand, double* initial_theta, int n, int dts, int log, int p_grid,
                           int q_grid) {
    ...
    if (strcmp(data->kernel_fun, "bivariate_matern_parsimonious2") == 0 ||
	    ...
	    EXAGEOSTAT_MLE_dcmg_Tile_Async(ChamUpperLower, chameleon_desc_submatrix(CHAM_descC, 0, 0, CHAM_descC->m / 2,
				    CHAM_descC->n / 2), &data->l1, &data->l1,
			    &data->lm, univariate_theta, &data->ns_params,data->dm,
			    "univariate_matern_stationary",data->Nsplit , msequence, mrequest);
	    ...
	    EXAGEOSTAT_MLE_dcmg_Tile_Async(ChamUpperLower, chameleon_desc_submatrix(CHAM_descC, CHAM_descC->m / 2,
				    0, CHAM_descC->m / 2, CHAM_descC->n / 2),
			    &data->l1, &data->l1, &data->lm,
			    univariate2_theta,&data->ns_params, data->dm, "univariate_matern_stationary",data->Nsplit , msequence, mrequest);
	    ...
	    EXAGEOSTAT_MLE_dcmg_Tile_Async(ChamUpperLower, chameleon_desc_submatrix(CHAM_descC, CHAM_descC->m / 2,
				    ...
				    CHAM_descC->n / 2), &data->l1, &data->l1,
			    &data->lm, univariate3_theta,&data->ns_params, data->dm, "univariate_matern_stationary",data->Nsplit , msequence,
			    mrequest);
    ...
    EXAGEOSTAT_g_to_ng_Tile_Async(CHAMELEON_descZ, initial_theta, msequence, mrequest);
    VERBOSE(" Done.\n");
...
int
EXAGEOSTAT_TLR_MLE_dzvg_ng_Tile_Async(MLE_data *data, double* Nrand, double* initial_theta, int n, int dts, int log, int p_grid,
		int q_grid) {
	//! Generate Observations Vector (Z) for testing Maximum
	...
	//Generate the co-variance matrix C
	VERBOSE("LR: Initializing Covariance Matrix (Synthetic Dataset Dense Generation Phase).....");
	EXAGEOSTAT_MLE_dcmg_Tile_Async(ChamLower, CHAM_descC, &data->l1, &data->l1, &data->lm, initial_theta,&data->ns_params, data->dm,
...
double EXAGEOSTAT_TLR_dmle_ng_Tile(unsigned n, const double* theta, double* grad, void *app_data)
	 ...
	 * */
{
	...
	HICMA_desc_t *hicma_descproduct = (HICMA_desc_t *) data->hicma_descproduct;    
	...
	HICMA_desc_t *hicma_descsum = (HICMA_desc_t *) data->descsum;
	...
	if (strcmp(data->recovery_file, "") != 0 &&
		...
		if (strcmp(data->kernel_fun, "univariate_matern_non_gaussian") == 0
			...
			//CHAMELEON_Tile_to_Lapack(cham_descproduct, z, 1);
			//HICMA_Lapack_to_Tile(z, 1, hicma_descproduct);
			//free(z);
			EXAGEOSTAT_ng_transform_lr_Tile_Async(hicma_descZ, hicma_descproduct, theta, hsequence,
					hrequest);
			...
			{
				double *z = (double *) malloc(1 * N * sizeof(double));
			...
			EXAGEOSTAT_ng_loglike_lr_Tile_Async(hicma_descZ, hicma_descsum, theta, hsequence, hrequest);
			CHAMELEON_Sequence_Wait(hsequence);
			VERBOSE(" Done.\n");
		}
		...
		HICMA_dtrsmd_Tile(ChamLeft, ChamLower, ChamNoTrans, ChamNonUnit, 1, hicma_descCUV, hicma_descCD, hicma_descCrk,
				hicma_descZ, maxrank);
		...
		VERBOSE(" Done.\n");
		VERBOSE("LR:Calculating dot product...");
		//printf("%d, %d, %d\n", cham_descZ->nb, cham_descproduct->nb, hicma_descZ->nb);
		CHAMELEON_dgemm_Tile(ChamTrans, ChamNoTrans, 1, cham_descZ, cham_descZ, 0, cham_descproduct);

		loglik = -0.5 * ((MLE_data *) data)->dotp - 0.5 * logdet;// - (double) (N / 2.0) * log(2.0 * PI);

		loglik = loglik - data->sum - N * log(theta[3]) - (double) (N / 2.0) * log(2.0 * PI); //theta[3] = omega;
		VERBOSE(" Done.");

		//Calculate the ranks
		if (calc_rank_stat_ng == 1) {
			int i = 0;
			int MBrk = 1;
			int NBrk = 1;
			...
			if (flag2 == 0) {
				flag2 = 1;
				Ark_initial_ng = (double* ) calloc(Mrk * Nrk, sizeof(double));
				Ark_old_ng = (double* ) calloc(Mrk * Nrk, sizeof(double));
			}
			CHAMELEON_Tile_to_Lapack(hicma_descCrk, Ark_initial_ng, Mrk);
			for (i = 0; i < Mrk * Nrk; i++) {
				if (Ark_initial_ng[i] > Ark_old_ng[i]) {
					Ark_old_ng[i] = Ark_initial_ng[i];
				}
			}

			if (CHAMELEON_My_Mpi_Rank() == 0) {
				fwrite_array(hicma_descCrk->m, hicma_descCrk->n, hicma_descCrk->m, Ark_old_ng, "ranks.csv");
				print_array(hicma_descCrk->m, hicma_descCrk->n, hicma_descCrk->m, Ark_old_ng, stdout);
				HICMA_stat_t hicma_statrk_initial;
				zget_stat(ChamLower, Ark_old_ng, Mrk, Nrk, Mrk, &hicma_statrk_initial);
	...
	((MLE_data *) data)->avg_generation_per_iter += matrix_gen_time;
	((MLE_data *) data)->avg_cholesky_per_iter += time_facto;
	((MLE_data *) data)->avg_solve_per_iter += time_solve;
...
void EXAGEOSTAT_TLR_dmle_ng_Predict_Allocate(MLE_data *CHAM_data, int nZmiss, int nZobs, int lts, int p_grid, int q_grid,
		int mse_flag)
	 ...
	 * */
{
	...
	EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&CHAMELEON_descr, NULL, ChamRealDouble, lts, lts, lts * lts, nZobs, nZmiss, 0, 0,
			nZobs, nZmiss, p_grid, q_grid);

	EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&CHAMELEON_descrcpy, NULL, ChamRealDouble, lts, lts, lts * lts, nZobs, nZmiss, 0, 0,
			nZobs, nZmiss, p_grid, q_grid);
	...
	EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&CHAM_descC12, NULL, ChamRealDouble, lts, lts, lts * lts, nZmiss, nZmiss, 0, 0,
...
//Predict missing values base on a set of given values and covariance matrix
	double
EXAGEOSTAT_TLR_dmle_ng_Predict_Tile(MLE_data *CHAM_data, double* theta, int nZmiss, int nZobs, double* Zobs, double* Zactual,
	 ...
	 * */
{
	...
	HICMA_descproduct = data->descproduct;
	...
	EXAGEOSTAT_ng_transform_Tile_Async(CHAMELEON_descZobs, HICMA_descproduct, theta, hsequence, hrequest);
	...
	int index;
	// I need to change the location struct to vector array (TO DO)
	for (index = 0; index < nZobs; index++) {
		xycoord[index] = data->lobs.x[index];
		xycoord[nZobs + index] = data->lobs.y[index];
	}
	...
	EXAGEOSTAT_MLE_dcmg_Tile_Async(ChamLower, CHAMELEON_descr, &data->lobs, &data->lmiss,
			&data->lm, theta,&data->ns_params, data->dm, data->kernel_fun, data->Nsplit ,hsequence, hrequest);
	CHAMELEON_Sequence_Wait(hsequence);
	VERBOSE(" Done.\n");
	...
	HICMA_dtrsmd_Tile(ChamLeft, ChamLower, ChamNoTrans, ChamNonUnit, 1, hicma_descC22UV, hicma_descC22D,
			hicma_descC22rk, CHAMELEON_descr, maxrank);
	HICMA_dtrsmd_Tile(ChamLeft, ChamLower, ChamTrans, ChamNonUnit, 1, hicma_descC22UV, hicma_descC22D, hicma_descC22rk,
			CHAMELEON_descr, maxrank);
	...
	if(CHAMELEON_My_Mpi_Rank() == 0)
	{
		...
		snprintf(nFileZ, 50, "%s%f%s%f%s%f%s", "./predict_tlr_", results.initial_theta[0], "_", results.initial_theta[4],
				"_", results.initial_theta[5], ".log");
...
void EXAGEOSTAT_TLR_dmle_ng_Call(MLE_data *data, int ncores, int gpus, int lts, int p_grid, int q_grid, int N, int nZobs, int nZmiss)	//! //Initiate HICMA and allocate different descriptors for
	/*!  HICMA
	 * Returns MLE_data data with initial values and new descriptors locations.
	 * @param[in] data: MLE_data struct with different MLE inputs.
	 ...
	 * */
{
	...
	HICMA_desc_t *hicma_descproduct = NULL;
	CHAM_desc_t *CHAMELEON_descZmiss = NULL;
	CHAM_desc_t *CHAM_descmse = NULL;
	CHAM_desc_t *CHAMELEON_descZactual = NULL;
	CHAM_desc_t *CHAMELEON_descZobs = NULL;
	...
	HICMA_desc_t *hicma_descsum = NULL;
	int MBC, NBC, MC, NC;
	int MBD, NBD, MD, ND;
	int MBUV, NBUV, MUV, NUV;
	int MBrk, NBrk, Mrk, Nrk;
	...
	HICMA_Desc_Create(&hicma_descZcpy, Zcpy, HicmaRealDouble, lts, lts, lts * lts, N, 1, 0, 0, N, 1,
			p_grid, q_grid);
	//	EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&descproduct, &data->dotp, ChamRealDouble, dts, dts, dts * dts, 1, 1, 0, 0, 1,
	//			1, p_grid, q_grid);
	HICMA_Desc_Create(&hicma_descproduct, &data->dotp, HicmaRealDouble, lts, lts, lts * lts, 1, 1, 0, 0, 1,
			1, p_grid, q_grid);
	...
	HICMA_Desc_Create(&hicma_descsum, &data->sum, HicmaRealDouble, lts, lts, lts * lts, 1, 1, 0, 0, 1,
			1, p_grid, q_grid);
