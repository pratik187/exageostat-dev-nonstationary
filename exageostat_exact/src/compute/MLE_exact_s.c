     ...
     * */
{
    ...
    MORSE_MLE_scmg_Tile_Async(MorseLower, data->descC, &data->l1, &data->l1, &data->lm, initial_theta, data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    VERBOSE(" Done.\n");
...
double MORSE_smle_Tile(unsigned n, const double * theta, double * grad, void * MORSE_data) {
    ...
    MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC, &data->l1, &data->l1, &data->lm, (double *)theta, data->dm, data->kernel_fun,data->Nsplit ,   msequence, &mrequest[0]);
     ...
     * */
{
    ...
    MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC22, &data->lobs, &data->lobs, &data->lm, theta, data->dm, data->kernel_fun,data->Nsplit ,   msequence, mrequest);
    MORSE_Sequence_Wait(msequence);
    ...
    if(MORSE_My_Mpi_Rank() == 0)
    {
        ...
        write_prediction_result("predict_result.dat", n, data->hicma_acc, 0, 0, 0, data->mserror, (mat_gen_time+time_solve+ time_gemm), (flops / 1e9 / (time_solve)));
     ...
     * */
{
    ...
    VERBOSE("Generate C12 Covariance Matrix... (Prediction Stage - single precision)");
    //MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC12, msequence, mrequest,  &data->lmiss, &data->lobs, theta, data->dm);
    MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC12, &data->lmiss, &data->lobs, &data->lm, theta, data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    MORSE_Sequence_Wait(msequence);
    VERBOSE(" Done.\n");
    STOP_TIMING(mat_gen_time);
