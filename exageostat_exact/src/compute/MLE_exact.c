     ...
     * */
{
    ...
    // double *sigma_arr = calloc(n, sizeof(double));
    // double *lambda_arr = calloc(n, sizeof(double));
    ...
    // parameter_array(sigma_arr, lambda_arr, data->Nsplit, n, &data->l1, &data->lm, initial_theta);
    // for(int i =0; i<n; i++){
    //     printf("%f \t %f \n",sigma_arr[i],lambda_arr[i]);
    // }
    // exit(0);
    //MORSE_MLE_dcmg_Tile_Async(MorseLower, data->descC, msequence, mrequest, &data->l1, &data->l1, initial_theta, data->dm);

    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,n,n,&data->l1,&data->l1,&data->lm,initial_theta);
    }
    ...
    if(strcmp(data->kernel_fun, "bivariate_matern_parsimonious2")   == 0 || strcmp(data->kernel_fun, "bivariate_matern_parsimonious2_profile")   == 0)
    {
        ...
        MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, data->descsubC11, &data->l1, 
                &data->l1, &data->lm, univariate_theta, &data->ns_params,
                data->dm, "univariate_matern_stationary",data->Nsplit , msequence, &mrequest[0]);
        ...
        MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, data->descsubC12, &data->l1, 
                &data->l1, &data->lm, univariate2_theta, &data->ns_params,
                data->dm, "univariate_matern_stationary", data->Nsplit ,  msequence, &mrequest[0]);
        ...
        MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, data->descsubC22, &data->l1, &data->l1, 
                &data->lm, univariate3_theta, data->dm, &data->ns_params,
                "univariate_matern_stationary", data->Nsplit ,  msequence, &mrequest[0]);
    }
    else	if(strcmp(data->kernel_fun, "univariate_matern_non_stationary")   == 0)
    {
        MORSE_MLE_dcmg_Tile_Async(MorseLower, data->descC, &data->l1,
                &data->l1, &data->lm, initial_theta, &data->ns_params,
                data->dm, "univariate_matern_stationary", data->Nsplit ,  msequence, mrequest);
    }
    ...
    else
    {
        MORSE_MLE_dcmg_Tile_Async(MorseLower, data->descC, &data->l1,
                &data->l1, &data->lm, initial_theta, &data->ns_params,
                data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    }
    ...
    // exit(0);
    	...
    	//  double *C = (double *) malloc(n * n * sizeof(double));
    	//  MORSE_Tile_to_Lapack( data->descC, C, n);
    	//  print_dmatrix("syntheticC", 8, 8, C, 8);
    //exit(0);
    //Copy Nrand to Z
    ...
    if(log==1)
    {
        ...
        CHAMELEON_Tile_to_Lapack( MORSE_descZ, z, n);
...
double MORSE_dmle_Tile(unsigned n, const double * theta, double * grad, void * MORSE_data) {
    ...
    int N, NRHS, success, i, num_params, split;
    double flops =0.0;	
    double* univariate_theta;
    double* univariate2_theta;
    double* univariate3_theta;
    ...
    MORSE_desc_t *MORSE_descZ3       = (MORSE_desc_t *) data->descZ3;
    ...
    MORSE_desc_t *MORSE_descproduct3 = (MORSE_desc_t *) data->descproduct3;
    MORSE_sequence_t *msequence	 = (MORSE_sequence_t *) data->sequence;
    MORSE_request_t  *mrequest	 = (MORSE_request_t *) data->request;
    data->dotp= 0;
    data->dotp1 =0;
    ...
    N	= MORSE_descC->m;
    NRHS	= MORSE_descZ->n;
    // printf("%d parameters in morse_dmle_tile\n", num_params);
    // for(int i = 0;i<6;i++){
    //     printf("%f___\n",theta[i]);
    // }
    ...
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,N,N,&data->l1,&data->l1,&data->lm,theta);
    }
    // full_data l;
    // //memory allocation
    // l.z       = (double *) malloc(N * sizeof(double));
    // MORSE_Tile_to_Lapack( data->descZ, l.z, N);
    ...
    // for ( g = 0; g < 3; g++) {
    //     int c = 10;
    //     ptr[g] = &c; /* assign the address of integer. */
    // }
    ...
    // MORSE_Tile_to_Lapack( data->descZ, Z, N);
    // print_dmatrix("before Z transform (Data Generation)", 16, 1, Z, 1);
    // exit(0);
    ...
    // double split = 4;
    //memory allocation
    // l.x     = (double *) malloc(N * 1 * sizeof(double));
    // l.y     = (double *) malloc(N * 1 * sizeof(double));
    ...
    // for(loop = N ; loop > 300 ; loop--)
    //     printf("%f,%f,%f ", l.x[loop],l.y[loop],Z[loop]);
    // // printf("%f,%f,%f", l.x[256],l.y[256],Z[256]);
    // exit(0);
    // int splitX = 2;
    // int splitY = 2;
    // full_data l;
    ...
    // l.z       = (double *) malloc(N * sizeof(double));
    // MORSE_Tile_to_Lapack(data->descZ, l.z, N);
    // data_split_nonstat_userDefined(l.x,l.y,l.z, &data, N, splitX,splitY);
    // double sum1 =0;
    // double sum2 = 0;
    // for(int i=0;i<N;i++){
    //     sum1+=l.x[i];
    ...
    else
    {
        ...
        if(strcmp(data->kernel_fun, "bivariate_matern_parsimonious2")   == 0 || strcmp(data->kernel_fun, "bivariate_matern_parsimonious2_profile")   == 0)
        {
            ...
            MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_descsubC11, &data->l1,
                    &data->l1, &data->lm, univariate_theta, data->dm, &data->ns_params,
                    "univariate_matern_stationary",data->Nsplit ,   msequence, &mrequest[0]);
            ...
            MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_descsubC22, &data->l1, 
                    &data->l1, &data->lm, univariate3_theta, &data->ns_params,
                    data->dm, "univariate_matern_stationary", data->Nsplit ,  msequence, &mrequest[0]);
            //MORSE_Sequence_Wait(msequence);
        }
            ...
            MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC, &data->l1,
                    &data->l1, &data->lm, (double *)theta,&data->ns_params, data->dm, 
                    data->kernel_fun, data->Nsplit , msequence, &mrequest[0]);
        MORSE_Sequence_Wait(msequence);
        STOP_TIMING(matrix_gen_time);
        VERBOSE(" Done.\n");
        ...
        // double *C = (double *) malloc(N * N * sizeof(double));
        // MORSE_Tile_to_Lapack( data->descC, C, N);
        // print_dmatrix("testC_estim", 16, 16, C, 16);
        // exit(0);
        ...
        //dotp=0;
        MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ, MORSE_descZ,MORSE_descproduct, index, increment, msequence, &mrequest[0]);
        MORSE_Sequence_Wait(msequence);        
        ...
        if(strcmp(data->kernel_fun, "bivariate_matern_parsimonious2_profile")   == 0 )
        {
            ...
            //MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ1, MORSE_descZ1, MORSE_descproduct1, 0, 2, msequence, &mrequest[0]);

            //MORSE_Sequence_Wait(msequence);

            //MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ2, MORSE_descZ2, MORSE_descproduct2, 1, 2, msequence, &mrequest[0]);
        ...
        else if(strcmp(data->kernel_fun, "bivariate_matern_parsimonious_profile")   == 0)
        {
            ...
            loglik = -(N /2.0) + (N /2.0)*log (N/2.0) -(N / 2.0 ) * log(data->dotp) -  0.5*logdet - (double) (N / 2.0) * log(2.0 * PI);
            //     MORSE_Sequence_Wait(msequence);
            //to be optimized
            ...
            MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ, MORSE_descZ, MORSE_descproduct1, 0, 2, msequence, &mrequest[0]);

            MORSE_Sequence_Wait(msequence);

            MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ, MORSE_descZ, MORSE_descproduct2, 1, 2, msequence, &mrequest[0]);

            MORSE_Sequence_Wait(msequence);
            data->variance1 = (1.0/(N/2.0)) * data->dotp1;
            data->variance2 = (1.0/(N/2.0)) * data->dotp2;

            //	printf("%f, %f, %f, %f\n", data->dotp1, data->dotp2, data->dotp3, data->dotp);
            //exit(0);
        }
        else if(strcmp(data->kernel_fun, "trivariate_matern_parsimonious_profile")   == 0)
        {
            ...
            loglik = -(N /2.0) + (N /2.0)*log (N/2.0) -(N / 2.0 ) * log(data->dotp) -  0.5*logdet - (double) (N / 2.0) * log(2.0 * PI);
            //to be optimized
            //MORSE_tristride_vec_Tile_Async(MORSE_descZ, MORSE_descZ1, MORSE_descZ2, MORSE_descZ3, msequence, &mrequest[0]);
            //*********************************
            //MORSE_dgemm_Tile (MorseTrans, MorseNoTrans, 1, MORSE_descZ1, MORSE_descZ1, 0, MORSE_descproduct1);
            //MORSE_dgemm_Tile (MorseTrans, MorseNoTrans, 1, MORSE_descZ2, MORSE_descZ2, 0, MORSE_descproduct2);
            //MORSE_dgemm_Tile (MorseTrans, MorseNoTrans, 1, MORSE_descZ3, MORSE_descZ3, 0, MORSE_descproduct3);


            MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ, MORSE_descZ, MORSE_descproduct1, 0, 3, msequence, &mrequest[0]);

            MORSE_Sequence_Wait(msequence);

            MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ, MORSE_descZ, MORSE_descproduct2, 1, 3, msequence, &mrequest[0]);

            MORSE_Sequence_Wait(msequence);

            MORSE_MLE_ddotp_Async(MorseTrans, MorseNoTrans, MORSE_descZ, MORSE_descZ, MORSE_descproduct3, 2, 3, msequence, &mrequest[0]);
...
// double MORSE_dmle_Tile_nonstat(unsigned n, const double * theta, double * grad, void * MORSE_data) {
//     //! Maximum Likelihood Evaluation (MLE)
//     /*!  -- MORSE-sync
//      * Returns the loglikelihhod value for the given theta.
//      * @param[in] n: unsigned variable used by NLOPT library.
...
double MORSE_dmle_Tile_Async(unsigned n, const double * theta, double * grad, void * MORSE_data) {
    ...
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC, &data->l1, &data->l1, &data->lm, (double *)theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,   msequence, &mrequest[0]);
    MORSE_Sequence_Wait(msequence);
    STOP_TIMING(matrix_gen_time);
    VERBOSE(" Done.\n");
     ...
     * */
{
    ...
    MORSE_desc_t *MORSE_descmse3     = NULL;
    ...
    if(strcmp(data->kernel_fun, "bivariate_matern_parsimonious")   == 0 ||
            ...
            strcmp(data->kernel_fun, "bivariate_spacetime_matern_stationary")   == 0  )
    {
    ...
    if( mse_flag == 1)
    {
        ...
        EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descmse3, &data->mserror3, MorseRealDouble, dts, dts, dts * dts, 1, 1, 0, 0, 1, 1, p_grid, q_grid);
    }
     ...
     * */
{
    ...
    if(strcmp(data->kernel_fun, "univariate_matern_stationary")   == 0 || strcmp(data->kernel_fun, "univariate_pow_exp_stationary")   == 0 )
        ...
        num_params = 10;
    else if(strcmp(data->kernel_fun, "trivariate_matern_parsimonious")   == 0 || strcmp(data->kernel_fun, "trivariate_matern_parsimonious_profile")   == 0)
        num_params = 10;
    else if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0){
        split = data->Nsplit;
        num_params = (int)3*split ;
    ...
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZmiss,nZmiss,&data->lmiss,&data->lmiss,&data->lm,theta);
    }
    //MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC22, msequence, mrequest,  &data->lobs, &data->lobs, theta, data->dm);
    MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_descC11, &data->lmiss, &data->lmiss, &data->lm, theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    MORSE_Sequence_Wait(msequence);
    ...
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZobs,nZmiss,&data->lobs,&data->lmiss,&data->lm,theta);
    }
    //MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC22, msequence, mrequest,  &data->lobs, &data->lobs, theta, data->dm);
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC21, &data->lobs, &data->lmiss, &data->lm, theta, &data->ns_params,
        data->dm, data->kernel_fun, data->Nsplit ,   msequence, mrequest);
    MORSE_Sequence_Wait(msequence);
    //flops = flops + FLOPS_DPOTRF(nZobs);
    VERBOSE(" Done.\n");
    ...
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZobs,nZobs,&data->lobs,&data->lobs,&data->lm,theta);
    }
    //MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC22, msequence, mrequest,  &data->lobs, &data->lobs, theta, data->dm);
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC22, &data->lobs, &data->lobs, &data->lm, theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    MORSE_Sequence_Wait(msequence);
    //flops = flops + FLOPS_DPOTRF(nZobs);
    VERBOSE(" Done.\n");
    ...
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZmiss,nZobs,&data->lmiss,&data->lobs,&data->lm,theta);
    }
    //MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC12, msequence, mrequest,  &data->lmiss, &data->lobs, theta, data->dm);
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC12, &data->lmiss, &data->lobs, &data->lm, theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,   msequence, mrequest);
    MORSE_Sequence_Wait(msequence);
    ...
    //    MORSE_dposv_Tile(MorseLower, MORSE_descC22, MORSE_descC21);
    ...
    //   double *C11_arr = (double *) malloc(nZmiss * nZmiss * sizeof(double));
    //    MORSE_Tile_to_Lapack( MORSE_descC11, C11_arr, nZmiss);
    //     print_dmatrix("Schur_complement\n", nZmiss, nZmiss, C11_arr, nZmiss);
    ...
    // for(i=0;i<nZmiss;i++)
    //    sum+=pow((Ztrace[i] - mean), 2);
    // sum = sqrt(sum/(nZmiss-1));
    ...
    if( Zactual != NULL)
    {
...

        {
            ...
            data->mserror  /= nZmiss;
            data->mserror1 /= (nZmiss/2.0);
            data->mserror2 /= (nZmiss/2.0);
        }
        else if(strcmp(data->kernel_fun, "trivariate_matern_parsimonious")   == 0 || strcmp(data->kernel_fun, "trivariate_matern_parsimonious_profile")   == 0)
        {
            MORSE_MLE_dmse_trivariate_Tile_Async(MORSE_descZactual, MORSE_descZmiss, MORSE_descmse1, MORSE_descmse2, MORSE_descmse3, MORSE_descmse,  msequence, mrequest);
            MORSE_Sequence_Wait(msequence);
            VERBOSE(" Done.\n");
            STOP_TIMING(time_mse);
            data->mserror  /= nZmiss;
            data->mserror1 /= (nZmiss/3.0);
            data->mserror2 /= (nZmiss/3.0);
            data->mserror3 /= (nZmiss/3.0);       
        }
        else
        {
    ...
    if(MORSE_My_Mpi_Rank() == 0)
    {
        ...
        write_prediction_result("predict_result.dat", nZmiss, data->hicma_acc, data->mserror1, data->mserror2, data->mserror3, data->mserror, (mat_gen_time+time_solve+ time_gemm), (flops / 1e9 / (time_solve)));
        ...
        results.mse_pred3 = data->mserror3;
        results.mse_pred  = data->mserror;
        results.trace_pred_mean = mean;
        results.trace_pred_sum  = sum;
     ...
     * */
{
    ...
    VERBOSE("Generate C22 Covariance Matrix... (Prediction Stage)");
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZmiss,nZmiss,l2,l2,&data->lm,theta);
    }
    //MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC22, msequence, mrequest,  l2, l2, theta, data->dm);
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC22, l2, l2, &data->lm, theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,   msequence, mrequest);
    //flops = flops + FLOPS_DPOTRF(nZobs);
    VERBOSE(" Done.\n");
    ...
    VERBOSE("Generate C12 Covariance Matrix... (Prediction Stage)");
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZobs,nZmiss,l1,l2,&data->lm,theta);
    }
    //MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC12, msequence, mrequest,  l1, l2, theta, data->dm);
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC12, l1, l2, &data->lm, theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    //flops = flops + FLOPS_DPOTRF(nZmiss);
    VERBOSE(" Done.\n");
    STOP_TIMING(mat_gen_time);
     ...
     * */
{
    ...
    MORSE_desc_t *MORSE_descmloe            = NULL;
    MORSE_desc_t *MORSE_descmmom            = NULL;
    ...
    data->mloe=0;
    data->mmom=0;
    EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descmloe, &data->mloe, MorseRealDouble, dts, dts, dts * dts, 1, 1, 0, 0, 1, 1, p_grid, q_grid);
    EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descmmom, &data->mmom, MorseRealDouble, dts, dts, dts * dts, 1, 1, 0, 0, 1, 1, p_grid, q_grid);
    ...
    data->descmloe              = MORSE_descmloe;
    data->descmmom              = MORSE_descmmom;
     ...
     * */
{
    ...
    MORSE_desc_t *MORSE_descmloe              = data->descmloe;
    MORSE_desc_t *MORSE_descmmom              = data->descmmom;
    ...
    */
    ...
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZobs,nZobs,&data->lobs,&data->lobs,&data->lm,estimated_theta);
    }
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descK_a,  &data->lobs, &data->lobs, &data->lm, estimated_theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    // }
    MORSE_Sequence_Wait(msequence);
    //(2)Generate the co-variance matrix descK_t
    if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZobs,nZobs,&data->lobs,&data->lobs,&data->lm,truth_theta);
    }
    MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descK_t,  &data->lobs, &data->lobs, &data->lm, truth_theta, &data->ns_params, 
        data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
    MORSE_Sequence_Wait(msequence);
    VERBOSE(" Done.\n");
    ...
    //Cholesky factorization for the Co-variance matrix MORSE_descK_a
    START_TIMING(cholesky1);
    VERBOSE("(3)Cholesky factorization of MORSE_descK_a (MLOE-MMOM) .....");
    ...
    //VERBOSE("(4)Copy MORSE_descK_t Covariance Matrix to MORSE_descK_ttmp  (MLOE-MMOM).....");
    ...
    VERBOSE("(5)Cholesky factorization of MORSE_descK_t (MLOE-MMOM) .....");
    ...
    for(p=0; p<nZmiss; p++)
    {
        ...
        if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZobs,nZmiss,&data->lobs,&lmiss,&data->lm,truth_theta);
        }
        MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_desck_t, &data->lobs, &lmiss, &data->lm, truth_theta, &data->ns_params, 
            data->dm, data->kernel_fun, data->Nsplit ,  msequence, mrequest);
            if(strcmp(data->kernel_fun, "univariate_matern_non_stat_estimation")   == 0 ){
        parameter_array(data,nZobs,nZmiss,&data->lobs,&lmiss,&data->lm,estimated_theta);
        }
        MORSE_MLE_dcmg_Tile_Async(MorseUpperLower, MORSE_desck_a, &data->lobs, &lmiss, &data->lm, estimated_theta, &data->ns_params, 
            data->dm, data->kernel_fun,data->Nsplit , msequence, mrequest);
        MORSE_Sequence_Wait(msequence);
        STOP_TIMING(vecs_gen);


        //(6a)Copy MORSE_desck_a to MORSE_descK_atmp  (MLOE-MMOM)
        VERBOSE("(6a)Copy MORSE_desck_a to MORSE_descK_atmp  (MLOE-MMOM).....");
        ...
        START_TIMING(gevv2);
        //(12) Calculate dgemm value= MORSE_desck_t^T * MORSE_desck_a
        ...
        STOP_TIMING(gevv2);
        START_TIMING(gevv3);
        //(13) Calculate dgemm value= MORSE_desck_a^T * MORSE_desck_atmp
        ...
        START_TIMING(gevv1);
        //(11) Calculate dgemm value= MORSE_desck_a^T * MORSE_desck_t
        ...
        //VERBOSE("(14)Calculate 2dtrmm MORSE_desck_a = MORSE_descK_ttmp * MORSE_desck_a... (MLOE-MMOM)");
        //MORSE_dgemm_Tile(MorseNoTrans, MorseNoTrans, 1, MORSE_descK_ttmp, MORSE_desck_a, 0, MORSE_desck_t);
        START_TIMING(gevv4);
        MORSE_dtrmm_Tile(MorseLeft, MorseLower, MorseTrans, MorseNonUnit, 1, MORSE_descK_t, MORSE_desck_a);
        //MORSE_dtrmm_Tile(MorseLeft, MorseLower, MorseNoTrans, MorseNonUnit, 1, MORSE_descK_t, MORSE_desck_a);
        ...
        //MORSE_dtrsm_Tile(MorseLeft, MorseLower, MorseNoTrans, MorseNonUnit, 1, MORSE_descK_a, MORSE_desck_atmp);
        ...
        //MORSE_dtrsm_Tile(MorseLeft, MorseLower, MorseTrans, MorseNonUnit, 1, MORSE_descK_a, MORSE_desck_atmp);
        //flops = flops + FLOPS_DTRSM(MorseLeft, MORSE_descK_a->m, MORSE_desck_t->n);
        //VERBOSE(" Done.\n");
        //STOP_TIMING(trsm6);
        //START_TIMING(gevv5);

        //MORSE_dtrmm_Tile(MorseLeft, MorseLower, MorseTrans, MorseNonUnit, 1, MORSE_descK_t, MORSE_desck_atmp);
        ...
        MORSE_dgemm_Tile(MorseTrans, MorseNoTrans, 1, MORSE_desck_a, MORSE_desck_a, 0, MORSE_descexpr2);
        ...
        //     		printf ("%f, %f\n", matrix_gen3, matrix_gen4);
        printf ("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n", trsm1, trsm2, trsm3, trsm5, trsm6, gevv1, gevv2, gevv3, gevv4, gevv5);
        //*******************************
        ...
        MORSE_MLE_dmloe_mmom_Tile_Async(MORSE_descexpr2, MORSE_descexpr3, MORSE_descexpr4, MORSE_descmloe, MORSE_descmmom,  msequence, mrequest);
        MORSE_Sequence_Wait(msequence);


        // double *expr2_ = (double *) malloc(4 * sizeof(double));
        //  MORSE_Tile_to_Lapack( MORSE_descexpr2, expr2_, 4);
        //  double *expr3_ = (double *) malloc(4 * sizeof(double));
        //  MORSE_Tile_to_Lapack( MORSE_descexpr3, expr3_, 4);
        //   double *expr4_ = (double *) malloc(4 * sizeof(double));
        //   MORSE_Tile_to_Lapack( MORSE_descexpr4, expr4_, 4);

        //        for(i=0;i<4; i++)
        //          printf("%f- %f -%f\n", expr2_[i], expr3_[i], expr4_[i]);
        //    exit(0);
     ...
     * */
{
    ...
    for(p=0; p<nZmiss; p++)
    {
        ...
        //MORSE_dgemm_Tile(MorseNoTrans, MorseNoTrans, 1, MORSE_descK_ttmp, MORSE_desck_a, 0, MORSE_desck_t);
        START_TIMING(gevv4);
        MORSE_dtrmm_Tile_Async(MorseLeft, MorseLower, MorseTrans, MorseNonUnit, 1, MORSE_descK_t, MORSE_desck_a, msequence, mrequest);
        //MORSE_dtrmm_Tile(MorseLeft, MorseLower, MorseNoTrans, MorseNonUnit, 1, MORSE_descK_t, MORSE_desck_a);
        flops = flops + FLOPS_DGEMM(MORSE_desck_ttmp->m, MORSE_desck_a->n, MORSE_desck_t->n);
        VERBOSE(" Done.\n");
        STOP_TIMING(gevv4);
        ...
        //(15) Triangular Solve (TRSM) k_atmp = TRSM(K_a^-1, k_atmp)
        //	VERBOSE("(15)Solving the linear system k_atmp = TRSM(K_t^-1, k_atmp) ...\n");
        //	MORSE_dtrsm_Tile_Async(MorseLeft, MorseLower, MorseNoTrans, MorseNonUnit, 1, MORSE_descK_a, MORSE_desck_atmp, msequence, mrequest);
        //	flops = flops + FLOPS_DTRSM(MorseLeft, MORSE_descK_a->m, MORSE_desck_t->n);
        //	VERBOSE(" Done.\n");
        //	STOP_TIMING(trsm5);
        ...
        //(16) Triangular Solve (TRSM) k_a = TRSM(K_a^-T, k_a)
        //	VERBOSE("(16)Solving the linear system k_atmp = TRSM(K_t^-T, k_atmp) ...\n");
        //	MORSE_dtrsm_Tile_Async(MorseLeft, MorseLower, MorseTrans, MorseNonUnit, 1, MORSE_descK_a, MORSE_desck_atmp, msequence, mrequest);
        //	flops = flops + FLOPS_DTRSM(MorseLeft, MORSE_descK_a->m, MORSE_desck_t->n);
        //	VERBOSE(" Done.\n");
        //	STOP_TIMING(trsm6);
        START_TIMING(gevv5);

        //	MORSE_dtrmm_Tile_Async(MorseLeft, MorseLower, MorseTrans, MorseNonUnit, 1, MORSE_descK_t, MORSE_desck_atmp, msequence, mrequest);
        //(13) Calculate dgemm value= MORSE_desck_a^T * MORSE_desck_t
        VERBOSE("(17)Calculate dgemm MORSE_descexpr1 = MORSE_desck_a^T * MORSE_desck_a... (Prediction Stage)");
        MORSE_dgemm_Tile_Async(MorseTrans, MorseNoTrans, 1, MORSE_desck_a, MORSE_desck_a, 0, MORSE_descexpr2, msequence, mrequest);
        flops = flops + FLOPS_DGEMM(MORSE_desck_atmp->m, MORSE_desck_t->n, MORSE_descexpr2->n);
        VERBOSE(" Done.\n");
        STOP_TIMING(gevv5);
     ...
     * */
{
    ...
    MORSE_desc_t *MORSE_descZ3       = NULL;
    ...
    MORSE_desc_t *MORSE_descproduct3 = NULL;
    MORSE_desc_t *MORSE_descdet	 = NULL;
    //MORSE_desc_t *MORSE_descZmiss	 = NULL;
    //MORSE_desc_t *MORSE_descC12	 = NULL;
    //MORSE_desc_t *MORSE_descC22	 = NULL;
    ...
    // int k = 4;
    // MORSE_desc_t *descsubC[k];
    // double nodes_row[k];
    // double nodes_column[k];
    // int i;
    // double sqrt(double);
    ...
    EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descproduct3, &data->dotp3, MorseRealDouble, dts, dts, dts * dts, 1, 1, 0, 0, 1, 1, p_grid, q_grid);
    EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descdet, &data->det, MorseRealDouble, dts, dts, dts * dts, 1, 1, 0, 0, 1, 1, p_grid, q_grid);


    // for( i = 0; i < sqrt(k); i ++)
    // {
    //     nodes_row[i] = i*(MORSE_descC->n)/sqrt(k);
    //     nodes_column[i] = i*(MORSE_descC->m)/sqrt(k);
    //     // printf("column value : %f, row value : %f",nodes_column[i],nodes_row[i]);
    // }
    // // void MORSE_descsubC[k];
    // for(i = 0; i < sqrt(k); i ++)
    // {
    //     // MORSE_desc_t *MORSE_descsubC   = NULL;
    //     descsubC[i] = morse_desc_submatrix(MORSE_descC, nodes_column[i], nodes_row[i], nodes_column[1], nodes_row[1]);
    //     //ptr[i] = &MORSE_descsubC[i];
    // }
    ...
    if(strcmp(data->kernel_fun, "bivariate_matern_parsimonious_profile")   == 0 || strcmp(data->kernel_fun, "bivariate_matern_parsimonious2_profile")   == 0
            || strcmp(data->kernel_fun, "bivariate_matern_parsimonious")   == 0 || strcmp(data->kernel_fun, "bivariate_matern_parsimonious2")   == 0)

        p =2 ;
    else if(strcmp(data->kernel_fun, "trivariate_matern_parsimonious_profile")   == 0 || strcmp(data->kernel_fun, "trivariate_matern_parsimonious2_profile")   == 0
            || strcmp(data->kernel_fun, "trivariate_matern_parsimonious")   == 0 || strcmp(data->kernel_fun, "trivariate_matern_parsimonious2")   == 0)
        p =3;


    //EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descZ1, NULL, MorseRealDouble, dts, dts, dts * dts, N/p, 1, 0, 0, N/p, 1, p_grid, q_grid);
    //EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descZ2, NULL, MorseRealDouble, dts, dts, dts * dts, N/p, 1, 0, 0, N/p, 1, p_grid, q_grid);
    //EXAGEOSTAT_ALLOCATE_MATRIX_TILE(&MORSE_descZ3, NULL, MorseRealDouble, dts, dts, dts * dts, N/p, 1, 0, 0, N/p, 1, p_grid, q_grid);

    if( p != 1)
    {
        MORSE_descsubC11 = morse_desc_submatrix(MORSE_descC, 0,   0, MORSE_descC->m/p, MORSE_descC->n/p);
        MORSE_descsubC12 = morse_desc_submatrix(MORSE_descC, MORSE_descC->m/p,   0, MORSE_descC->m/p, MORSE_descC->n/p);
        //MORSE_descsubC21 = morse_desc_submatrix(MORSE_descC, 0,   MORSE_descC->m/2, MORSE_descC->m/2, MORSE_descC->n/2);
        MORSE_descsubC22 = morse_desc_submatrix(MORSE_descC, MORSE_descC->m/p,   MORSE_descC->n/p, MORSE_descC->m/p, MORSE_descC->n/p);
        // printf("address of subdesc = %d\n", MORSE_descsubC11);
        // printf("address of subdesc = %d\n", MORSE_descsubC12);
    }
    //if(nZmiss != 0)
    //{
    ...
    data->descproduct3      = MORSE_descproduct3;
    //data->descZmiss		= MORSE_descZmiss;
    //data->descC12		= MORSE_descC12;
    //data->descC22		= MORSE_descC22;
    //data->descmse		= MORSE_descmse;
