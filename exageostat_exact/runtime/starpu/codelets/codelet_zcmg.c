...
static void cl_dcmg_cpu_func(void *buffers[], void *cl_arg) {
    ...
    starpu_codelet_unpack_args(cl_arg, &m, &n, &m0, &n0, &l1, &l2, &lm, &theta, &ns_params, &distance_metric, &kernel, &Nsplit);//, &size);
    ...
    if (kernel == 0)
        ...
        core_dcmg_non_stat_estimation(A,m,n,m0,n0,l1,l2,lm,theta,ns_params,distance_metric,Nsplit);
}
...
int EXAGEOSTAT_MLE_dcmg_Tile_Async(CHAM_enum uplo, CHAM_desc_t *descA, location *l1,
                                  location *l2, location *lm, double* theta,non_stat_params *ns_params,
                                  char* dm, char* kernel_fun,int *Nsplit,
    ...
    if (strcmp(kernel_fun, "univariate_matern_stationary") == 0)
        ...
        kernel = 19;
    else if (strcmp(kernel_fun, "univariate_matern_non_stat_estimation") == 0)
        kernel = 20;
    else {
    ...
    for (n = 0; n < A.nt; n++) {
        ...
        for (; m < A.mt; m++) {
            ...
            starpu_insert_task(starpu_mpi_codelet(cl),
                               ...
                               STARPU_VALUE, &ns_params, sizeof(non_stat_params*),
                               STARPU_VALUE, &distance_metric, sizeof(int),
                               STARPU_VALUE, &kernel, sizeof(int),
                               STARPU_VALUE, &Nsplit, sizeof(int),
                               0);
        }
