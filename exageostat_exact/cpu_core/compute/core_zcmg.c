...
void core_dcmg_non_stat_estimation(double *A, int m, int n, int m0, int n0, location *l1, location *l2, location *lm, double *localtheta, non_stat_params *ns_params, int distance_metric, int Nsplit)
{   
    ...
    for (i = 0; i < m; i++) {
        j0 = n0;
        for (j = 0; j < n; j++) {
            double term1 = (ns_params->sigma_arr1[i0]) * (ns_params->sigma_arr2[j0]) * sqrt(ns_params->lamda_arr1[i0]) * sqrt(ns_params->lamda_arr2[j0]);
            double term2 = 2.0 / (ns_params->lamda_arr1[i0] + ns_params->lamda_arr2[j0]);
            double nui = ns_params->nu_arr1[i0];
            double nuj = ns_params->nu_arr2[j0];
            double neuij = (nui+nuj)/2;
            con = pow(2,(neuij-1)) * tgamma(neuij);
            con = 1.0/con;
            double Qij = pow(calculateDistance(l1, l2, i0, j0, distance_metric, 0),2)*term2;
            double prod1 = 2.0 * sqrt(neuij * Qij);
            
            if(Qij == 0.0)
                A[i + j * m] = term1*term2 /*+ 1e-4*/;
            else
                A[i + j * m] = term1*term2*con*pow(prod1, neuij)
                    * gsl_sf_bessel_Knu(neuij,prod1); // Matern Function
            j0++;
        }
        i0++;
    }
